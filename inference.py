...
from models.generator import LORT
    ...
    with torch.no_grad():
        for file_name in os.listdir(a.input_noisy_wavs_dir):
            if file_name.lower().endswith('.wav'):
                ...
                print(index)
                noisy_wav, sr = librosa.load(os.path.join(a.input_noisy_wavs_dir, file_name), h.sampling_rate)
                ...
                clean_wav, sr = librosa.load(os.path.join(a.input_clean_wavs_dir, file_name), h.sampling_rate)
...
def main():
    ...
    parser.add_argument('--checkpoint_file2', default='paper_result')
    a = parser.parse_args()

    config_file = os.path.join(a.checkpoint_file2, 'config.json')
