...
from src.LRC import DLC, DLC_dilated
from src.normalization import LayerNorm1, CLayerNorm, ScaleNorm
from src.conv_module import ConvModule
from src.attention import Attention1, Attention2
...
class Gated_FSMN_dilated(nn.Module):
    def __init__(
        ...
        self.to_u = FFConvM(
            dim_in=in_channels,
            dim_out=hidden_size,
            # norm_klass=nn.LayerNorm(in_channels),
            dropout=0.1,
        )
        self.to_v = FFConvM(
            dim_in=in_channels,
        ...
        self.time_fsmn = DLC_dilated(in_channels, out_channels, lorder, hidden_size)
        self.freq_fsmn = DLC_dilated(in_channels, out_channels, lorder, hidden_size)
    ...
    def forward(
        ...
        x_u = self.to_u(x)
        b, c, t, f = x_u.size()
        xt_u = x_u.permute(0, 3, 2, 1).contiguous().view(b*f, t, c)
        xt_u = self.time_fsmn(xt_u) + xt_u
        xf_u = xt_u.view(b, f, t, c).permute(0, 2, 1, 3).contiguous().view(b*t, f, c)
        xf_u = self.freq_fsmn(xf_u) + xf_u
        x_u_final = xf_u.view(b, t, f, c).permute(0, 3, 1, 2)
        x = x_v * x_u_final + input
        return x
...
class Gated_FSMN_Block_Dilated(nn.Module):
    ...
    def __init__(self,
        ...
        self.gated_fsmn = Gated_FSMN_dilated(dim, dim, lorder=10, hidden_size=dim)
    ...
    def forward(self, input):
        ...
        seq_out = self.gated_fsmn(norm1)
        norm2 = self.norm2(seq_out)
...
class refine_att(nn.Module):  # MSAR
    ...
    def forward(self, q, k, v, size):
        ...
        qk_concat = rearrange(qk_concat, "B h Ch2 H W -> B (h Ch2) H W", H=H, W=W)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias, shared_refine_att=None, qk_norm=1):
        ...
        self.channel_attn = Attention1(dim)
        self.spatial_attn = Attention2()
        self.norm = qk_norm
    ...
    def forward(self, x):
        ...
        refine_weight = self.refine_att(q, k, v, size=(h, w))  # G=(b×head×1×h×w)
        ...
        # csa
        out = out * channel_attn.expand_as(out) * spatial_attn
...
class MFEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.fsmn = nn.ModuleList([Gated_FSMN_Block_Dilated(dim) for _ in range(self.num_layers)])
...
class Multi_transformer(nn.Module):
    def __init__(self,
                 ...
                 out_channels=2,
                 num_blocks=[4, 4, 4, 4],
                 dec_num_blocks=[4, 4, 4, 4],
                 num_refinement_blocks=2,
                 heads=[1, 2, 4, 8],
                 ffn_expansion_factor=2.66,
                 bias=False,
                 num_path=[1, 1, 1, 1],
                 dec_num_path=[1, 1, 1, 1],
                 qk_norm=1,
                 offset_clamp=(-1, 1)
                 ):
